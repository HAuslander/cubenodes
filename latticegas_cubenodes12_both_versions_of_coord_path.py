...
class cubenodeset_t():
    ...
    def DivideCube(self, CoordNode, NodeCoord):
    ...
    def BruteFindCubeCoord(self, Id, nbrsubset, pathdump_index_subset, pathdump, pathdumpgeneration, gen0len, TestForInteriorPoints, sortbyones=None):
        ...
        def PruneASingleContenderx(SubCoord_pathx, pathdump):
            ...
            for sum1, key in ones_keys:
                ...
                #pathstarts = [pathdump[ipath][0] for ipath in SubCoord_pathx[key][0][0].values()]

                for isubkey in subkeys:
                    dellist = []
                    for iscenario in range(len(SubCoord_pathx[isubkey])):
                        if len(SubCoord_pathx[isubkey]) < 2:
                            continue                        
                        if iscenario in dellist:
                            continue
                        thisendpt = SubCoord_pathx[isubkey][iscenario][-1]
                        pathind = SubCoord_pathx[key][iscenario][0][isubkey]
        ...
        for iic, iipath in enumerate(pathdump_index_subset):        
            thiscoord = sortbyones[1 + iic][1]
            #coord_path[thiscoord] = [({zerocoord:iipath}, pathdump[iipath][-1])] # note that as written, the list will only have
        ...
        if len(good_coordnode_nodecoord_pairs) > 1:
            ...
            #if Id == 1087:
            #    print("1087 returns 1 good pair for ", nbrsubset)
            #    import pdb; pdb.set_trace()
            return newcoordnode, newnodecoord
        else:
    ...
    def ExpandManyRandomly(self, Prob, NRuns):
        ...
        for i in range(NRuns):
            ...
            for inode in ishuf: #range(bottomrun, toprun):
                ...
                dellist = []
                for iielement, ielement in enumerate(coordnode_nodecoord_list):
                ...
                coords, noodes = coordnode_nodecoord_list[0]
                ...
                self.BruteDivideCube(coords, noodes)
